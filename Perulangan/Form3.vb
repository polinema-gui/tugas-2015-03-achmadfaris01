Public Class Form3
    ...
    Private Sub ButtonProses_Click(sender As Object, e As EventArgs) Handles ButtonProses.Click
        Try
            Dim barisMatriks1 As Integer = TextBoxMatrix1.Lines.Count
            Dim kolomMatriks1 As Integer = TextBoxMatrix1.Lines.ElementAt(0).Split(" ").Length
            Dim barisMatriks2 As Integer = TextBoxMatrix1.Lines.Count
            Dim kolomMatriks2 As Integer = TextBoxMatrix1.Lines.ElementAt(0).Split(" ").Length
        ...
        If (Me.cekKolom) Then
            For baris As Integer = 0 To baris1 - 1
                For kolom As Integer = 0 To kolom1 - 1
                    nilai1(baris, kolom) = TextBoxMatrix1.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                    nilai2(baris, kolom) = TextBoxMatrix2.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                Next
            Next
        End If
    End Sub
    ...
    Private Sub cek(ByVal baris1 As Integer, baris2 As Integer, kolom1 As Integer, kolom2 As Integer)
        ...
        For baris = 0 To baris1 - 2
            If (TextBoxMatrix1.Lines.ElementAt(baris).Split(" ").Length <> TextBoxMatrix1.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        For baris = 0 To baris2 - 2
            If (TextBoxMatrix2.Lines.ElementAt(baris).Split(" ").Length <> TextBoxMatrix2.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        If (tmp = False) Then
            Me.cekKolom = False
            Convert.ToInt32(TextBoxMatrix1.Text)
            Convert.ToInt32(TextBoxMatrix2.Text)
